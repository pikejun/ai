 ...
 */
public class LstmLayer implements Serializable
{
    int inputWidth ;
    int stateWidth ;
    ...
    public LstmLayer(int input_width,int state_width,double learning_rate)
    {
        ...
        if(times==0)
        {
          // CalcOperator.sigmoid(CalcOperator.matrixMultiply(W_fx, input))
           ...
           currentState = CalcOperator.dot(io, co);
            ...
            currentOutput =CalcOperator.softMax(CalcOperator.dot (cto, oo)) ;
        }
        else
        {
            f_net=CalcOperator
